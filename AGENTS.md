...
其中 `q_i` 是一个标量，表示沿已给定温度依赖误差包络的整体位移。该参数化是基线接口，不构成方法创新。ETR25 的 low/median/high 来自 actual rate PDF 的百分位，而 factor uncertainty 使用 log-normal approximation；两者必须在 BBN 温区和最终 abundance/posterior 层面比较。每个 nuclear-input draw 必须生成跨温度 coherent 的 rate curve，禁止无协方差依据的独立 temperature-bin noise。
