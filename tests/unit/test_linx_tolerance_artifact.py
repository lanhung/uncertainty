...
RUN = ROOT / "artifacts/numerical/LINX-BATCH-TOLERANCE-v1/run-20260722T0618Z"
...
def test_tolerance_manifest_is_bound_to_frozen_inputs() -> None:
...
def test_all_cases_completed_but_frozen_plateaus_failed() -> None:
    ...
    ] == pytest.approx(0.004044721112909704)
    assert decision["plateaus"]["weak_rate_sampling"][
        "maximum_difference_observation_sigma"
    ] == pytest.approx(0.028684212946358173)


def test_raw_timing_and_failure_evidence_is_complete() -> None:
    records = [
        json.loads(line)
    ...
    assert resources["estimated_cost_cny"] == pytest.approx(0.5758585061475634)
